...
def render(cfg):
    ...
    # Save GT poses
    save_gt_positions(f"{cfg.data_collection.saved_dir}/gt_poses.txt",
                      list_position, cfg.data_collection)

    # Create directories for data
    rgb_dir = create_directory(
        f"{cfg.data_collection.saved_dir}/rgb", ignore_request=True)
    depth_dir = create_directory(
        f"{cfg.data_collection.saved_dir}/depth", ignore_request=True)
    semantic_dir = create_directory(
        f"{cfg.data_collection.saved_dir}/semantic", ignore_request=True)
    ...
    logging.info(f"Rendered scene at: {cfg.data_collection.saved_dir}")


def render_directory(cfg):
    logging.warning(f"running: {cfg.script}")
